...
class ControlTask(Task):
    ...
    def evaluate(self, model, n_iterations, deterministic=True, promptless_eval=False):
        ...
        for i in range(n_iterations):
            ...
            # sample prompt
            input_dict = self.sample_batch_configurable(batch_size=1, device=model.device, prompt_proportions=[1.], prompt_types = ['end'], max_tokens = model.module.context_len, share_prompt_episodes=True,ep_ids=self.top_ids)[0]
